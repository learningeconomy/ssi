...
impl std::fmt::Display for BBSErrorKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::KeyGenError => write!(f, "Key Generation Error"),
            Self::PublicKeyGeneratorMessageCountMismatch(expected, _) => write!(
                f, "Public key to message mismatch. Expected {}, found {}", expected, expected
            ),
            Self::SignatureIncorrectSize(found) => {
                write!(f, "Signature incorrect size. Expected 193, found {}", found)
            }
            Self::SignatureValueIncorrectSize => {
                write!(f, "Signature cannot be loaded due to a bad value")
            }
            Self::MalformedPublicKey => write!(f, "Malformed public key"),
            Self::PoKVCError { msg } => write!(f, "Error from PoKVC module {:?}", msg),
            Self::InvalidNumberOfBytes(expected, _) => {
                write!(f, "Invalid number of bytes. Expected {}, found {}", expected, expected)
            }
            Self::InvalidProof { status } => write!(f, "The proof failed due to {}", status),
