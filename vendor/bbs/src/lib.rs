// Modified for SSI LC-2160: adapt rand 0.8 only at pairing-plus RNG boundaries.
...
random_elem_impl!(GeneratorG1, { Self(G1::random(&mut Rng06(thread_rng()))) });
#[cfg(feature = "wasm")]
...
random_elem_impl!(SignatureMessage, {
    Self(Fr::random(&mut Rng06(thread_rng())))
});
#[cfg(feature = "wasm")]
...
random_elem_impl!(ProofNonce, { Self(Fr::random(&mut Rng06(thread_rng()))) });
#[cfg(feature = "wasm")]
...
random_elem_impl!(ProofChallenge, {
    Self(Fr::random(&mut Rng06(thread_rng())))
});
#[cfg(feature = "wasm")]
...
random_elem_impl!(SignatureBlinding, {
    Self(Fr::random(&mut Rng06(thread_rng())))
});
#[cfg(feature = "wasm")]
