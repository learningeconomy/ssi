...
// The published libipld 0.14 derive leaves the unknown-field decode type implicit.
// Preserve its map codec, including unit-only unknown values and length limits,
// while making that type explicit for current Rust. Fields are in CBOR key order.
macro_rules! cacao_map_codec {
    ($($field:ident: $name:literal $(= $default:expr)?),+ $(,)?) => {
        fn encode<W: std::io::Write>(
            ...
            let len = [$(stringify!($field)),+].len() as u64
                $(- cacao_map_codec!(@omitted self.$field $(, $default)?))+;
            libipld::cbor::encode::write_u64(w, libipld::cbor::cbor::MajorKind::Map, len)?;
            $(
                cacao_map_codec!(@encode self.$field, $name, c, w $(, $default)?);
            )+
            Ok(())
        }
    };
    (@omitted $value:expr, $default:expr) => { u64::from($value == $default) };
    (@omitted $value:expr) => { 0 };
    (@encode $value:expr, $name:expr, $c:expr, $w:expr, $default:expr) => {
        if $value != $default {
            cacao_map_codec!(@encode $value, $name, $c, $w);
        }
    };
    (@encode $value:expr, $name:expr, $c:expr, $w:expr) => {
        libipld::codec::Encode::encode($name, $c, $w)?;
        libipld::codec::Encode::encode(&$value, $c, $w)?;
    };
}

macro_rules! cacao_map_decode {
    ($($field:ident: $name:literal $(= $default:expr)?),+ $(,)?) => {
        fn decode<R: std::io::Read + std::io::Seek>(
            ...
            use libipld::cbor::{
                ...
                error::{LengthOutOfRange, UnexpectedCode},
            };
            ...
            if len > [$(stringify!($field)),+].len() as u64 {
                return Err(LengthOutOfRange::new::<Self>().into());
            }
            $(let mut $field = None;)+
            for _ in 0..len {
                let key: String = libipld::codec::Decode::decode(c, r)?;
                match key.as_str() {
                    $($name => { $field = Some(libipld::codec::Decode::decode(c, r)?); })+
                    _ => { let _: () = libipld::codec::Decode::decode(c, r)?; }
                }
            }
            Ok(Self {
                $($field: cacao_map_decode!(@value $field, $name $(, $default)?)),+
            })
        }
    };
    (@value $value:expr, $name:expr, $default:expr) => { $value.unwrap_or($default) };
    (@value $value:expr, $name:expr) => {
        $value.ok_or(libipld::cbor::error::MissingKey::new::<Self>($name))?
    };
}
...
impl<S, T> libipld::codec::Encode<libipld::cbor::DagCborCodec> for CACAO<S, T>
where
    S: SignatureScheme<T>,
    S::Signature: DagCbor,
    T: Representation,
{
    cacao_map_codec!(h: "h", p: "p", s: "s");
}

impl<S, T> libipld::codec::Decode<libipld::cbor::DagCborCodec> for CACAO<S, T>
where
    S: SignatureScheme<T>,
    S::Signature: DagCbor,
    T: Representation,
{
    cacao_map_decode!(h: "h", p: "p", s: "s");
}
